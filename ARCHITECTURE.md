...
- **No escape hatch.** What covers a misbehaving script instead is `requestStop()`'s own SIGTERM → SIGKILL escalation, which handles anything that reaches the runner view.
