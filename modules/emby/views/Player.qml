...
FocusScope {
    ...
    function reportStopped(finalPositionMs, finalDurationMs, failed) {
        ...
        embyBackend.report_playback_stopped(itemId, mediaSourceId, msToTicks(pos), failed || false)
    }
    ...
    function advanceToEpisode(detail) {
        ...
        embyBackend.set_last_track_langs(carryAudioLang, carrySubLang === "__off__" ? "" : carrySubLang,
                                              aLangIdx, sLangIdx)
        ...
        embyBackend.get_playback_url(detail.itemId, detail.mediaSourceId || detail.itemId,
    ...
    Connections {
        ...
        function onErrorOccurred(msg) {
            console.log("[Emby Player] Backend error: " + msg)
        ...
        function onStreamUrlReady(url) {
            if (pendingNextEpisode) {
                ...
                if (introSkipSetting !== "Off" || outroSkipSetting !== "Off")
                    embyBackend.fetchSegments(playerRoot.itemId)
                return
            }
            if (pendingRetryTranscode) {
        ...
        function onPlaybackEnded(finalPositionMs, finalDurationMs, reason) {
            if (reason === "failed") {
                if (!isTranscoding) {
                    ...
                    embyBackend.get_playback_url(itemId, mediaSourceId, aIdx, sIdx, true)
                    return
                }
            ...
            if (reason === "eof" && autoplayNext) {
                pendingNextEpisode = true
                embyBackend.load_next_episode(itemId)
                return
            }
            goBack()
        }
    ...
    Timer {
        ...
        onTriggered: {
            ...
            if (pos > 0) {
                ...
                embyBackend.update_playback_progress(itemId, mediaSourceId,
                                                         msToTicks(pos), false)
            }
        }
    }
    ...
    Component.onCompleted: {
        ...
        if (introSkipSetting !== "Off" || outroSkipSetting !== "Off")
            embyBackend.fetchSegments(itemId)
    ...
    // without stopping), report stopped so Emby doesn't show this as
