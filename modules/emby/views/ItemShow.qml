...
FocusScope {
    ...
    Connections {
        ...
        function onSeriesNextUpReady(detail) {
            ...
            if (detail && detail.itemId) {
                ...
                if (s0) embyBackend.load_episodes(showRoot.item.itemId, s0.itemId)
        ...
        function onErrorOccurred(msg) {
            ...
            showRoot.playPending = false
            showRoot.fallbackPlay = false
            console.log("[Emby Show] Error: " + msg)
        }
    }
    ...
    Component.onCompleted: {
        isLoading = true
        focusRow = 0
        if (item.itemId) embyBackend.load_seasons(item.itemId)
    }
    ...
    Keys.onReturnPressed: {
        if (focusRow === 0) {
            ...
            playPending = true
            embyBackend.load_series_next_up(item.itemId)
        } else {
