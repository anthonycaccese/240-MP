...
FocusScope {
    ...
    Connections {
        ...
        function onItemLoaded(d) {
            ...
            if (!serverPrefsLoaded) {
                ...
                embyBackend.load_server_preferences()
            }
        ...
        function onErrorOccurred(msg) {
            console.log("[Emby Item] Error: " + msg)
            detailRoot.isLaunching = false
        }
    }

    Component.onCompleted: {
        if (item.itemId) embyBackend.load_item_detail(item.itemId)
        focusRow = 0
    ...
    // ------------------------------------------------------------------
    // Language display — uses Emby's own DisplayTitle when available
    // ------------------------------------------------------------------
    ...
    // Mirrors Emby's MediaStreamSelector.GetDefaultSubtitleStreamIndex so the
    ...
    function _saveCurrentLangs() {
        ...
        embyBackend.set_last_track_langs(aLang, sLang, aLangIdx, sLangIdx)
    }
    ...
    function applyLanguagePreferences(d) {
        ...
        var savedAudioLangIdx = embyBackend.get_last_audio_lang_idx()
        var savedSubLangIdx   = embyBackend.get_last_sub_lang_idx()
